...
月次レポート詳細（`/reports/[id]`）では、**AI下書きを12構成のフォームへ直接反映**できます。
「編集する」→「AIで下書き生成」→ 生成結果を各項目に反映 → 担当者が編集 →「承認して保存」という
human-in-the-loop フローで、AI の構造化出力（`## 番号. タイトル` 見出し）を
`src/lib/ai/report-sections.ts` が12セクションに分解して各テキスト欄へ流し込みます。
